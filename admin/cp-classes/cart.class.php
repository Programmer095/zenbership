...
class cart extends db
{
    ...
    function check_addition($id, $qty, $options = '', $force_qty = '0')
    {
        if (!empty($id)) {
            ...
            // Allow multiple
             if ($product['data']['max_per_cart'] != '0' && !empty($product['data']['max_per_cart'])) {
            ...
            // Require minimum?
           if ($product['data']['min_per_cart'] != '0' && !empty($product['data']['max_per_cart'])) {
                $in_cart = $this->find_product_in_cart($id, $options);
                if ($force_qty == '1') {
                    $combine = $qty;
                } else {
            ...
            foreach ($use_order['components'] as $item) {
                ...
                if ($item['data']['type'] != '1' && $gateway != 'gw_paypal') {
                    if ($skip_subs_invoices != '1') {
                        ...
                        $sub = $sub_class->create_subscription($item, $use_order['data']['id'], $member_id, $card_id, '0', '', $gateway, $mtype, $item['pricing']['total']);
                    }
                } else {
                    // Invoice payment
                ...
                // Add to sales logs
                $add_sale = $this->add_sale($force_session, $item['data']['id'], $item['pricing'], $sub['id']);
            }
            // Paying an invoice?
     ...
     */
    function add_sale($order_id, $product_id, $product_pricing, $subscription_id, $status = '1')
    {
